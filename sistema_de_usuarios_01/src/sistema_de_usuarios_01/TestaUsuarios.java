...
public class TestaUsuarios {
	...
	static void emailInvalido() {
		...
		// Se alterar para vazio o campo email ele exibe a mensagem de email vazio, se
		// digitar errado ele exibe a mensagem para digitar um email valido
		try {
			Administrador adm = new Administrador("Zeus", "zeus.com", "1");
			adm.visualizar();
		} catch (IllegalArgumentException e) {
			System.err.println("Erro no email do Admnistrador: " + e.getMessage());
		}
		try {
			Usuario u1 = new Usuario("joel", "batatinha.com");
			u1.visualizar();
		} catch (IllegalArgumentException e) {
			System.err.println("Erro no email do Usuário: " + e.getMessage());
		}
	}
